...
class ReplicaActorIntegrationTest {
    ...
    @Test
    void localPutUpdatesSnapshotAndBroadcastsOperation() {
        ...
        actor = new ReplicaActor(
                "nodeA",
                new BTree<>(3),
                new PeerRegistry("nodeA", Map.of("nodeA", "http://nodeA", "nodeB", "http://nodeB")),
                broadcaster,
                ackSender,
        ...
        assertEquals(1, broadcaster.sent.size());
        assertInstanceOf(PutOperation.class, broadcaster.sent.getFirst());
        assertTrue(ackSender.acks.isEmpty(), "Local put must not send ACK to itself");
    }
    ...
    void outOfOrderRemoteOperationsAreBufferedAndThenApplied() {
        ...
        assertEquals(2, ackSender.acks.size());
        assertTrue(ackSender.acks.stream().allMatch(AckRequest::success));
    }

    @Test
    void duplicateRemoteOperationDoesNotDuplicateVisibleValue() {
        TestBroadcaster broadcaster = new TestBroadcaster();
        TestAckSender ackSender = new TestAckSender();
        ...
        assertTrue(result.exists());
        assertFalse(result.conflict());
        assertEquals(List.of("v1"), result.values());
        assertEquals(2, ackSender.acks.size(), "Duplicate is ACKed again but must not change state");
        assertEquals(1.0, registry.find("crdt.replication.duplicate").tag("operation", "put").counter().count());
    }
